...
//INTERNAL IMPORT FOR NFT OPENZEIPLINE
...
contract NFTMarketplace is ERC721URIStorage{
    ...
    function createMarketItem(uint256 tokenId , uint256 price) private {
        require(price>0, "Price must be al best 1");
    ...
    function fetchMarketItem() public view returns(MarketItem[] memory){
        ...
        for(uint256 i=0; i<itemCount;++i){
            if(idMarketItem[i+1].owner == address(this)){
                uint256 curentId = i+1;
                MarketItem storage currentItem= idMarketItem[curentId];
                items[curentIndex] = currentItem;
                curentIndex +=1;
    ...
    //SINGULE USER ITEMS
    function fetchItemListsed() public view returns (MarketItem[] memory){
        uint256 totalCount = _tokenIds.current();
        uint256 itemCount = 0;
        uint currentIndex = 0;
